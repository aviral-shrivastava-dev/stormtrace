# StormTrace: The Learning Journey

This project was built lesson by lesson. Each lesson records what was
added, why it matters, the commands to run it, and an exercise. It is
kept as a lab notebook: several lessons describe bugs that shipped and
how they were found, which is the most useful part.

For what the project is and how to run it, see the
[README](../README.md).

> **Note on the lesson text.** These lessons were written as each stage was
> built, so numbers and code excerpts inside them are snapshots of that
> moment. Several were later found to be wrong and fixed; the corrections
> are recorded in [Lesson 25](#lesson-25-the-code-review) rather than by
> rewriting history. Treat the README and the generated
> `data\reports\research_summary.md` as current; treat everything below as
...
## Lesson 25: The Code Review

Lessons 1-24 built the platform. This lesson is what happened when the
whole thing was read end to end with fresh eyes, looking for places where
the code did not do what its own comments claimed.

Six defects were found. Four had been silently producing wrong numbers for
weeks. None of them announced themselves: every pipeline run reported
success, every quality check passed, and the dashboard was green.

That is the lesson. A pipeline that runs is not a pipeline that is
correct.

### The bug that inverted a finding

`validate_ori.py` computed the correlation between the reliability score
and the measured error rate like this:

```python
spearman(scores, [s["km_per_hour"] for s in scored if s["km_per_hour"] is not None])
```

`scores` had 1,654 entries. The filtered rate list had 1,557. The
`pearson` helper paired them with `zip`, which stops at the shorter list
without complaining — so score row 0 was compared against rate row 0, but
row 900's score was compared against a *different pair's* rate.

The stored result was **+0.07**: "the index has no relationship to error
rate." Recomputed with the rows correctly paired, it is **-0.30**: a
moderate correlation in exactly the direction the index was designed for.

The fix is two functions:

```python
def paired_values(records, x_key, y_key):
    """Return two aligned series, keeping only rows where both exist."""
    xs, ys = [], []
    for record in records:
        x, y = record.get(x_key), record.get(y_key)
        if x is None or y is None:
            continue          # drop the row from BOTH series
        xs.append(float(x))
        ys.append(float(y))
    return xs, ys
...
1. **Filter pairs, not series.** Any time two lists must stay row-aligned,
   build them together in one pass.
2. **Make the failure loud.** `zip` truncating is a language feature, not a
   bug, so the guard has to be written by hand. `zip(..., strict=True)`
   does the same job in Python 3.10+, and it is now used throughout.

### The pipeline that could not survive its own timeout

`run_pipeline.py` called `subprocess.run(..., timeout=300)`. Its docstring
explained calmly that "a step killed by timeout leaves no partial data."
But `TimeoutExpired` appeared nowhere in the file, so a slow step raised
an unhandled exception, printed a traceback, and logged nothing at all.
The claim was fiction.

Now a timeout is a step *result*, not a crash: status `timeout`,
`return_code: None`, whatever output the child managed, written to the
JSONL log like any other event. The run then stops in the same controlled
way as any failure.

### The lock with a race and a too-short fuse

```python
if LOCK_PATH.exists():      # check
    ...
LOCK_PATH.write_text(...)   # ...then act
```

Two runs could both pass the check before either wrote. The hourly
scheduled task overlapping a manual run is exactly the scenario, and
DuckDB allows only one writer.

`os.open(path, O_CREAT | O_EXCL)` makes creation atomic: the loser gets
`FileExistsError` and stands down. Separately, the 30-minute stale-lock
window was shorter than a legitimately slow run (12 steps x 300 s), so a
healthy run could have its lock stolen mid-flight. The window is now
derived from the step timeout, and each completed step refreshes the lock
so its timestamp tracks *progress* rather than start time.

### The Gold layer that could only ever see one day

This was the architectural one, and it hid in plain sight.

`lesson3_gold.sql` read `data/silver/noaa_*_latest.csv`. That file is
overwritten every run and holds NOAA's rolling 24-hour window. So no
matter how long the project collected, `gold_space_weather_minute` had
about one day in it — 1,436 rows — while `noaa_magnetic_history` had
67,743 rows spanning the entire collection period.

Everything downstream inherited the ceiling: hourly disturbance levels,
the storm comparison, and the ORI **environment factor**, which was a
constant `1.0` for every object in the database. The project's whole
research question is whether space weather degrades orbit data, and the
space-weather input to the answer was multiplying by one.

The rewrite reads the history tables instead. That immediately created a
problem the old query never had to solve: overlapping rolling-window
fetches store the same observed minute roughly six times, 18,643 duplicate
`(observed_at_utc, spacecraft)` pairs. So Gold now deduplicates
explicitly, keeping the most trustworthy version of each minute:

```sql
row_number() OVER (
    PARTITION BY date_trunc('minute', observed_at_utc)
    ORDER BY is_active_source DESC NULLS LAST,   -- NOAA's own flag
             quality_code ASC NULLS LAST,        -- lower is better
             snapshot_at_utc DESC                -- newest revision wins
)
```

Coverage went from 1,436 rows over one day to 7,930 rows over six days,
and the environment factor can finally vary. Whether it *predicts*
anything still needs a storm; that has not happened yet, and the report
says so.

**The general lesson: if you built an immutable history, read from it.**
A "latest" file is a convenience for the next step, never a source of
truth for analysis.

### The pillar that could not measure what it claimed

`gold_satnogs_activity` counted `COUNT(*)` where it meant
`COUNT(DISTINCT observation_id)`. Because history deliberately keeps every
version of an observation, 275 rows were really 211 observations, and one
day reported 50 where 32 existed. The inflated numbers went straight to
Prometheus.

The deeper problem was worse. Every one of those 275 rows had status
`'future'`, so `SUM(good_count)` was **0** — and it always would have
been. The collector fetched only the newest page of the SatNOGS listing,
which returns *upcoming* passes, and never revisited them. A metric named
"good observations" was structurally incapable of ever being non-zero.

Three changes fixed it:

1. Sweep the terminal statuses directly (`?status=good|bad|failed`), which
   is where the actual outcomes live.
2. Re-fetch by id (`?id=N`) those observations already in Bronze whose
   pass window has closed but whose status is still provisional — capped at
   10 per run, oldest first, so the request count stays bounded and polite.
3. Count distinct observations at their *newest* version, and report the
   good rate over **completed** passes only.

That last point matters more than it looks:

```sql
ROUND(100.0 * COUNT(*) FILTER (WHERE status = 'good')
      / NULLIF(COUNT(*) FILTER (WHERE status IN ('good','bad','failed')), 0), 1)
```

`NULLIF` makes the rate `NULL` when nothing has completed. The API omits
the metric entirely in that case rather than exporting `0`, because a
gauge reading zero says "everything failed" when the truth is "nothing has
finished yet". **An unknown value is not a zero value.** The dashboard
panel is allowed to be empty, and `verify_dashboard.py` reports that as an
expected gap rather than a failure.

### The 5,079 km "measurement"

The largest recorded propagation disagreement was 5,079 km. That is not a
measurement of orbital drag; it is SGP4 being used far outside its
validity envelope, or a bad element set. Nothing flagged it, and it fed
the medians, the Spearman statistics, and the ML training target.

Deleting such rows would destroy evidence. Keeping them silently corrupts
every statistic. So each measurement now carries a label:

```python
def measurement_quality(span_hours, total_km):
    if total_km > MAX_ANALYSIS_TOTAL_KM:      # 500 km
        return "extreme"
    if span_hours > MAX_ANALYSIS_SPAN_HOURS:  # 72 h
        return "long_span"
    return "ok"
```

Every consumer filters to `'ok'` and reports how many pairs it excluded.
Of 1,654 measured pairs, 33 are outside the envelope. The reported maximum
dropped from 5,079 km to 377 km, and the ML model stopped learning mostly
from outliers — its pooled Spearman went from 0.565 to 0.579 on a cleaner
target.

**Label, exclude, and report the count. Never silently drop, never
silently average in.**

### Tests, at last

`tests/` contained three `.pyc` files and nothing else. `git log -- tests`
was empty: the test files had never been committed. CI ran `py_compile`
and an import check, which verifies that files are syntactically valid
Python. That is why a `zip` misalignment shipped.

There are now 165 tests, aimed deliberately at the places where being
wrong is invisible:

| Area | Why it is dangerous |
|---|---|
| SGP4 unit conversions | wrong scaling still propagates, just to the wrong place |
| RIC decomposition | must preserve total magnitude and sign convention |
| `spearman` / `average_ranks` | the bug this lesson opened with |
| Kp tenths normalisation | 67 means 6.7; a missing `/10` gives an impossible 720 |
| Quality-gate operators | a typo used to invert the check silently |
| Lock and timeout handling | both were broken and both looked fine |
| Gold SQL, per transformation | run against in-memory DuckDB fixtures |

Two habits made the suite useful rather than decorative:

- **Every fixed bug got a test that fails on the old code.** A test named
  `test_mismatched_lengths_raise_instead_of_truncating` documents a real
  event, not a hypothetical.
- **No test touches the network.** Collectors are tested by stubbing
  `fetch_json`, so the provider policy holds on CI runners too.

### The smaller repairs

- **`check_quality.py`** had 800-character SQL lines packed into
  positional tuples, and `is_pass` treated any operator that was not
  exactly `"less_than_or_equal"` as greater-than — so a typo inverted a
  check. Checks are now `dataclass` instances and operators come from a
  lookup table that raises on an unknown name.
- **An orphan Bronze file.** `celestrak/active_*.json` (6.9 MB) matched no
  loader pattern, so it was unregistered, unloaded, and invisible. The
  invariant "every Bronze file is accounted for" had a hole shaped like a
  file extension. A new warning check reports it.
- **Hardcoded reference numbers.** `train_model.py` printed the index's
  Spearman scores from constants that had drifted from what
  `validate_ori.py` actually produced. It now reads them from the
  warehouse, so the comparison always describes the same data.
- **Secrets as module constants.** `minioadmin`, `admin`. Now read from
  the environment with local defaults, `.env.example` committed, `.env`
  ignored, and every compose port bound to `127.0.0.1`.
- **Windowed SQL instead of Python dicts.** The disagreement builder
  loaded all history into a dict to find consecutive pairs; a `LAG` window
  does it in the database. `executemany` replaced hand-built VALUES clauses
  carrying up to 8,000 bound parameters.
- **`summarize_history.py`** was bare module-level code with no `main()`
  and no error handling, unlike its seventeen siblings. Fixed.
- **`train_model.py` joined the pipeline** as an optional step (exit 2 when
  there is too little data to train honestly, which the orchestrator logs
  as a skip). ML was manual before, so the model could silently go stale.
- **A 1,874-line README** was simultaneously a README, a tutorial, a
  changelog, and this lab notebook, with the interesting findings buried
  past a PowerShell tutorial. It is now a short README; the lessons live in
  this file.

### What this lesson is really about

Every one of these defects lived in code with a confident comment above
it. The comments were not lies when they were written — they described the
intent. They became lies when the implementation drifted and nothing
checked.

Three things generalise:

1. **A green pipeline proves nothing about correctness.** Every run
   succeeded throughout. Wrong numbers do not raise exceptions.
2. **Read from your source of truth.** The Bronze layer was built with
   real care, then the Gold layer quietly read a throwaway CSV instead.
3. **If a comment makes a claim, write the test that holds it to account.**
   The instinct to document reasoning is a strength. Untested assertions
   decay into confident fiction.

### Lesson 25 Exercise

1. Run `python -m pytest -q`. Pick a test whose name describes a bug
   (`test_timeout_is_logged_as_a_step_failure_not_a_crash` is a good one),
   revert the fix in `src/`, and watch it fail. That failure is what the
   old code did in production.
2. Query `gold_propagation_disagreement` grouped by `measurement_quality`.
   Compute the median `total_km` with and without the `'ok'` filter, then
   decide for yourself whether excluding 33 of 1,654 pairs is defensible.
   Write down your reasoning.
3. Find one comment in `src/` that asserts a behaviour with no test behind
   it. Write the test. If it passes, you have documentation; if it fails,
   you have found the next Lesson 25.
